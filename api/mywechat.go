...
const GetInntelnuminfoUrl = "https://v.19790526.xyz/api/phonequery/?phonenum="
	...
	*/
		...
		if err != nil {
			...
			if err != nil {
				...
				if err != nil {
					...
					i := 0
					for range rows {
						tempstr := emoji.Parse(":newspaper_roll:")
						tempstr += js.Get("T1348647853363").GetIndex(i).Get("title").MustString()
						tempstr += ("(" + js.Get("T1348647853363").GetIndex(i).Get("source").MustString() + ")：\n")
						tempstr += (js.Get("T1348647853363").GetIndex(i).Get("digest").MustString() + "\n")
						i++
						if len(msg)+len(tempstr) < 2048 { //微信公众号回文本消息限制在2048字节大小
							msg += tempstr
						} else {
							break
						}
	...
	if IsNumber(textRequestBody.Content) { //收到的首位为1的7位以上全数字或者86打头的9位以上全数字
		if (len(textRequestBody.Content) >= 7 && strings.HasPrefix(textRequestBody.Content, "1")) || (len(textRequestBody.Content) >= 9 && strings.HasPrefix(textRequestBody.Content, "861")) {
			...
			buff, _ := HTTPGet(GetInntelnuminfoUrl + gettingnum) //用自建的号码归属地查询接口
			...
			*/
			...
			if teleinfo.Status == "success" {
				msg = emoji.Parse(":telephone_receiver:") + "号码" + textRequestBody.Content + "的信息： \n" + "归属地：" + teleinfo.Province + teleinfo.City + "(" + teleinfo.AreaZone + ")" + "\n运营商：" + teleinfo.CardType
			} else {
				msg = "查询失败，可能是未知号码或者新放号段数据库还没更新"
			}
